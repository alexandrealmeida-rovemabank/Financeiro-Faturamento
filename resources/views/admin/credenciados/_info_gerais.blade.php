<div class="row">
    ...
    <div class="col-md-6">
        ...
        @if($empresa->taxas)
            <p><strong>Ciclo:</strong> {{ $empresa->taxas->ciclo ?? 'N/A' }}</p>
            <p><strong>Prazo Nota Fiscal:</strong> {{ $empresa->taxas->prazo_nota_fiscal ?? 'N/A' }}</p>
            <p><strong>Período Recebimento:</strong> {{ $empresa->taxas->periodo_recebimento ?? 'N/A' }}</p>
            <p><strong>Posto/Bandeira:</strong> {{ $empresa->taxas->postoBandeira->nome ?? 'N/A' }}</p>
        @endif
    </div>
</div>
